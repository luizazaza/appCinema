<!-- Modal -->
<div class="modal fade" id="modalAlterarPol-{{$dadospoltrona->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content" style="color: #fff; background-color: #2a2b38; background-image: url('https://s3-us-west-2.amazonaws.com/s.cdpn.io/1462889/pat.svg'); background-position: bottom center; background-repeat: no-repeat;">
      <div class="modal-header">
        <h1 class="modal-title fs-5 " id="exampleModalLabel">Alterar Funcionário</h1>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
      <form method = "post" action="{{route('alterar-banco-poltrona',$dadospoltrona->id)}}">
    @method('PUT')
    @csrf
            <div class="mb-3 form-check">
                <label for="nomeInput" class="form-label">Nome do cliente:</label>
                <input type="text" name="nomeclie" value="{{$dadospoltrona->nomeclie}}" class="form-control" id="nomeInput" >
            </div>

            <div class="mb-3 form-check">
                <label for="emailInput" class="form-label">Número da poltrona</label>
                <input type="text" name="id" class="form-control" value="{{$dadospoltrona->id}}" id="polInput">
            </div>

            <button type="submit" class="btn-geral">Salvar</button>
        </form>




    </div>
      <div class="modal-footer">
        <button type="button" class="btn-geral" data-bs-dismiss="modal">Cancelar</button>
      </div>
    </div>
  </div>
</div>